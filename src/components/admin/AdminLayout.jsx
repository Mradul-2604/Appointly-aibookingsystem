...
export default function AdminLayout() {
    ...
    return (
        <div className="bg-[#f5f6f7] text-on-background min-h-screen">
            ...
            <aside className={`md:hidden flex flex-col h-screen w-72 fixed left-0 top-0 bg-white border-r border-outline-variant/10 p-5 gap-2 z-50 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <SidebarContent />
            </aside>

            {/* Mobile Top Header */}
            <header className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b border-outline-variant/10 flex items-center px-4 gap-3 z-30">
                <button
                    onClick={() => setSidebarOpen(true)}
                    className="p-2 rounded-xl hover:bg-[#f3f4f5] transition-colors text-[#191c1d]"
                >
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-base font-bold text-[#191c1d]">Appointly Admin</span>
                </div>
            </header>
            ...
            {/* Mobile bottom nav */}
            <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-outline-variant/10 flex justify-around items-center h-16 px-2 z-30">
                {navItems.map(item => (
                    <Link
                        key={item.path}
                        to={item.path}
